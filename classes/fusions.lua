...
function FusionClass:can_fuse(fusion,input)
    ...
    for x,l in ipairs(input) do
       for i,v in ipairs(fusion.input) do
        if v.target == l.config.center.key then current_value=current_value+(v.input_worth or 1) end
        end
    end
...
for x,l in ipairs(input) do
       for i,v in ipairs(fusion.input) do
        if v.target == l.config.center.key then table.insert(cards_to_destroy, l) current_value=current_value+(v.input_worth or 1) end
...
SMODS.BalatroFusions.Fusion = FusionClass